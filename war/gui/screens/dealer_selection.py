...
class DealerSelectionScreen:
    ...
    def setup_buttons(self):
        ...
        button_y_back = int(self.screen_height * 0.85)  # 85% da altura para botão voltar (mais seguro)
    ...
    def check_results(self):
        ...
        if len(winners) == 1:
            ...
            self.dealer_index = None
            # Os jogadores perdedores são automaticamente eliminados na próxima rodada
    ...
    def start_next_round(self):
        ...
        if self.dealer_index is None and self.rolls:
            ...
            max_roll = max(self.rolls.values())
            self.current_players = [player_idx for player_idx, roll in self.rolls.items() if roll == max_roll]
            self.round_number += 1
            self.rolls.clear()
            self.dice_animations.clear()
            self.final_rolls.clear()
    ...
    def render(self):
        ...
        title_rect = title_surface.get_rect(center=(self.screen_width//2, int(self.screen_height * 0.1)))
    ...
    def render_playing(self):
        ...
        title_rect = title_surface.get_rect(center=(self.screen_width//2, int(self.screen_height * 0.17)))
        self.screen.blit(title_surface, title_rect)
        
        subtitle_surface = self.font_small.render(subtitle, True, GRAY)
        subtitle_rect = subtitle_surface.get_rect(center=(self.screen_width//2, int(self.screen_height * 0.22)))
        ...
        if row_1_players > 0:
            ...
            start_x_1 = (self.screen_width - total_width_1) // 2 + dice_spacing // 2
            y_1 = int(self.screen_height * 0.32)
            
            for i in range(row_1_players):
                player_idx = self.current_players[i]
                self.render_player_dice(player_idx, start_x_1 + i * dice_spacing, y_1, dice_size)
        
        # Segunda fileira
        if row_2_players > 0:
            total_width_2 = (row_2_players * dice_spacing)
            start_x_2 = (self.screen_width - total_width_2) // 2 + dice_spacing // 2
            y_2 = int(self.screen_height * 0.48)
            
            for i in range(row_2_players):
                player_idx = self.current_players[row_1_players + i]
                self.render_player_dice(player_idx, start_x_2 + i * dice_spacing, y_2, dice_size)
    ...
    def render_player_dice(self, player_idx, x, y, dice_size):
        ...
        pygame.draw.rect(self.screen, border_color, dice_rect, border_width, border_radius=8)
        ...
        dice_text = self.font_large.render(str(dice_value), True, BLACK if dice_color == WHITE or dice_color == YELLOW else WHITE)
        ...
        name_rect = name_surface.get_rect(center=(dice_rect.centerx, dice_rect.bottom + 20))
    ...
    def render_results_overlay(self):
        ...
        # Fundo semi-transparente para o resultado - posição mais alta para não sobrepor botão
        overlay_y = int(self.screen_height * 0.6)
        overlay_height = int(self.screen_height * 0.1)
        overlay_rect = pygame.Rect(0, overlay_y, self.screen_width, overlay_height)
        ...
        if len(winners) == 1:
            ...
            result_text = f"🎉 {winner_config['name']} será o entregador de cartas! 🎉"
            result_color = PLAYER_COLORS.get(winner_config["color"], WHITE)
        else:
            winner_names = [self.players_config[idx]["name"] for idx in winners]
            result_text = f"Empate entre: {', '.join(winner_names)}"
            result_color = WHITE
        
        result_surface = self.font_medium.render(result_text, True, result_color)
        result_rect = result_surface.get_rect(center=(self.screen_width//2, overlay_y + overlay_height//2))
    ...
    def render_rolling(self):
        ...
        title_surface = self.font_medium.render(round_text, True, WHITE)
        title_rect = title_surface.get_rect(center=(self.screen_width//2, int(self.screen_height * 0.2)))
        self.screen.blit(title_surface, title_rect)
    ...
    def render_results(self):
        ...
        title_rect = title_surface.get_rect(center=(self.screen_width//2, int(self.screen_height * 0.18)))
        self.screen.blit(title_surface, title_rect)
    ...
    def render_buttons(self):
        ...
        for button_name in visible_buttons:
            ...
            # Fundo do botão
            pygame.draw.rect(self.screen, DARK_GRAY, button_rect, border_radius=8)
